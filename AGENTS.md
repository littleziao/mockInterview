...
- 除纯文档、格式化或非常小的单点修改外，不要只写单行提交信息。
- 涉及功能、修复、重构、测试、数据库结构、接口契约或跨前后端联动时，必须写提交正文。
- 提交正文至少说明：做了什么、影响范围、测试验证；如果关联 issue，也写明 issue 编号。
- 多个相关点用短列表表达，避免空泛描述，例如：

```text
feat: 实现简历历史和面试记录删除

- 保存每次成功解析后的简历分析记录，首页展示历史列表
- 支持删除已完成面试记录，并刷新复盘详情与趋势数据
- 补充后端边界测试和前端用户流测试
- 验证 npm run lint、npm run typecheck、npm run test
...
- `npm run lint` 通过后再提交。
- `git commit -m "..." -m "..."` 完成提交；非微小改动必须包含正文。
- `git push` 后发起 PR（如需）。
